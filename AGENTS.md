...
- The dashboard is the one part that is neither shell nor Go ([ADR 0031](docs/adr/0031-the-dashboard-is-built-into-the-factory-binary.md)): npm in `factory/ui`, whose build in `factory/ui/dist/app` the binary embeds, so the Go tests need `make ui` first. `factory/ui/dist` stays in git with a placeholder, because Go refuses an embed pattern that matches nothing, and `factory/go.mod` ignores `./ui/node_modules`, because npm packages ship Go files of their own. The browser test starts the real binary in fake mode twice and compares one approved screenshot.
