...
## Commands
- Gate: `make check` runs everything CI runs (shellcheck, `claude plugin validate --strict`, the standard check, python unittest); `make lint`, `make validate`, `make standard`, `make test` run one part
