...
- A `blocked:` line becomes an `issue` finding with the fix the script printed, except the missing CI job named `check`: the cleanup pull request adds it, so the `tests-ci` auditor owns it.
