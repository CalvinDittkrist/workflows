...
| (called by `cleanup.sh prepare`) | `scaffold.sh [--skip <category>]... [--name <repo>] [--default <branch>] [<root>]` | create README.md, AGENTS.md, a CLAUDE.md that imports it, a Makefile with a `check` target, the CI job `check` when no workflow has one, `.github/dependabot.yml`, docs/architecture.md, docs/adr/, docs/glossary.md, PR template; never overwrites a file. `.claude/settings.json` gets the marketplace and the workflow plugins through `claude plugin ... --scope project`, other project plugins disabled, and the template's attribution, env and permissions merged in |
...
- `report.sh [<file>...]` reads the auditors' replies, keeps the lines `finding: <category> | <target> | <action> | <reason> | <confidence>`, fails on a malformed one, and prints the report per category: counts per action, what it deletes, what the run performs exactly as listed, what a script decides for itself and what that approval triggers, what becomes an issue. It stores the findings in `<git dir>/standardize/findings` and clears earlier approvals.
...
- `finalize.sh` refuses until the cleanup pull request is merged. Then it applies the workspace when that category was approved, names where the applied difference deviates from the audited one, comments the snapshot on the catalogue issue, removes the worktree and the branch, and runs `check.sh` on the default branch. It prints `result: pass` or `result: fail`.
