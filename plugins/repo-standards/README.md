...
| (called by `cleanup.sh prepare`) | `scaffold.sh [--skip <category>]... [<root>]` | create AGENTS.md, a CLAUDE.md that imports it, a Makefile with a `check` target, the CI job `check` when no workflow has one, `.github/dependabot.yml`, docs/architecture.md, docs/adr/, docs/glossary.md, PR template; never overwrites a file. `.claude/settings.json` gets the marketplace and the workflow plugins through `claude plugin ... --scope project`, other project plugins disabled, and the template's attribution, env and permissions merged in |
...
- `cleanup.sh prepare` refuses without the backup, creates or resumes the worktree `.claude/worktrees/chore-standardize`, deletes the approved targets there and runs `scaffold.sh`. It prints `todo:` lines for the agent. `cleanup.sh open` refuses while a `<fill in>` placeholder is left, then commits, pushes and opens or updates the pull request.
- `issues.sh` opens one `ready-for-agent` issue per approved issue finding and keeps the ones an earlier run opened.
- `finalize.sh` refuses while the cleanup pull request is open. Once it is merged, it applies the workspace when that category was approved, comments the snapshot on the catalogue issue, removes the worktree and the branch, and runs `check.sh` on the default branch. It prints `result: pass` or `result: fail`.
