...
2. Fill the created file: Context (forces, links), Decision (one decision, present tense), Consequences (including the rejected alternative). Five to twenty lines total. Set `Status: accepted` only if the user confirmed the decision; otherwise leave `proposed`.
