...
8. End with finalize.sh's `result:` line, every `check: fail:` line, the `workspace:` line about the applied difference deviating from the audited one when it printed one, the `untouched:` line naming the rejected categories, and any `manual:` steps from the scripts.
