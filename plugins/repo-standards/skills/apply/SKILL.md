...
1. If the approvals above start with `error:` or list a `pending` category, relay that and stop; the audit (`/repo-standards:standardize`) comes first. An `unanswered:` category does not stop the run: name those categories and say that the run scaffolds their missing baseline files, as it would on an approval, because only a rejection keeps it out; for `agent-config` say as well that the run brings `.claude/settings.json` to the template: the workflow plugins enabled, every other project plugin disabled, the template permissions and env merged.
