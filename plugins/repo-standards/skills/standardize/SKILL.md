...
6. End with the approve.sh output and one line: nothing in the repository or on GitHub has changed; the approved findings are stored for the apply phase. If workspace.sh printed an `error:`, add one line that the GitHub workspace was not audited, with that error.
