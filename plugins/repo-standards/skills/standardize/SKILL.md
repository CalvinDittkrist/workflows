...
5. Show the report as it is, including the lines that say what approving a category triggers. Then ask the user, for each category in the report, to approve or reject it; one answer may cover several categories, and a single finding cannot be left out. Record every answer with `"${CLAUDE_PLUGIN_ROOT}/scripts/approve.sh" <category>=approve|reject ...` and show its output. Ask again for categories still `pending`.
