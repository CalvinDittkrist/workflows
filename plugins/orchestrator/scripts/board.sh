...
if [ -n "$nwo" ]; then
  # Plan branches are dropped first: plan/<slug> carries a topic, so a slug starting with a number is no issue.
  claimed=$(git worktree list --porcelain | sed -nE '\#^branch refs/heads/plan/#d; s#^branch refs/heads/##p' \
