...
- `/orchestrator:hunt-tests` starts a worker that removes the tests of this repository that prove nothing and opens one pull request for them (`--sandbox`, `--base <branch>`).
  - Run it only when the user asks.
- `/orchestrator:board` shows every active worktree, its agent state, PR and checks.
  - It also shows the frontier: `ready-for-agent` issues that are unblocked and unclaimed, with their milestone.
