...
The agent has only the Bash tool and does not load CLAUDE.md; `/orchestrator:gh-axi` and `/orchestrator:herdr` are typed by you. Scripts require `HERDR_ENV=1`, `gh`, `jq`, `git`; `--sandbox` requires `sbx`. `WF_CLAUDE_ARGS` adds flags to every worker and planner start, `WF_PLANNER_CLAUDE_ARGS` and `WF_WORKER_CLAUDE_ARGS` to one kind; each started session disables the other workflow plugins through `--settings` so only its own skills and agents load; `WF_PLANNER_PERMISSION_MODE` (default `auto`) sets the planner's permission mode; `WF_PLANNER_LANGUAGE` (for example `german`) sets the language the planner talks in, through claude's `language` setting in that session's `--settings` (the issues it writes stay English). Claude Code keeps only the last `--settings`, so a `--settings` in `WF_CLAUDE_ARGS` or `WF_PLANNER_CLAUDE_ARGS` replaces that object, language included; both scripts warn when they see one.
