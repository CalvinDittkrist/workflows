...
# Usage: handoff-resume.sh <pane> <session-id-before> <stage> <driver-command>
# Started by handoff.sh with nohup, because the session it clears is the one that started it: a worker
# cannot clear itself from inside a turn. It waits until that worker is idle, sends `/clear`, and only when
...
pane="${1:-}"; before="${2:-}"; stage="${3:-}"; cmd="${4:-}"
[ -n "$pane" ] && [ -n "$before" ] && [ -n "$stage" ] && [ -n "$cmd" ] ||
  wf_die "usage: handoff-resume.sh <pane> <session-id-before> <stage> <driver-command>"
