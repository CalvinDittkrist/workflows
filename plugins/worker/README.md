...
Documentation: `scripts/claude-docs.sh` is the worker's one way to the network. Without an argument it prints the index of the Claude Code documentation, with a page slug (lowercase letters, digits and hyphens) that page; it builds the URL from a hard-coded origin, speaks https only and prints nothing when the answer came from elsewhere. `WF_DOCS_TIMEOUT` (default 30 s) is the request timeout.
