...
A round here is a repair round when the CI stage drove it, and that stage counts it in the repair record before it invokes this skill. A round somebody asked for by hand — a maintainer who read the pull request and requested changes — is a new mandate on it and not another turn of the pipeline's own loop, so it starts the count again. Which of the two this is, is not yours to judge: run `"${CLAUDE_PLUGIN_ROOT}/scripts/repair.sh" reset` before the first fix and read its answer. It starts the count again when the driver that began this session names the review the session is here for, once for that review, and otherwise keeps the count and says so, which is no error and nothing to report — the rounds the CI stage then drives on this pull request are the pipeline's own. That script is the one place the count lives, so this skill never keeps a second one.
