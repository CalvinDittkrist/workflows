...
When the session facts carry a `resume_stage:`, an earlier context of this worker handed the pipeline over to you and the SessionStart hook has injected its note: start at that stage, not at stage 1. Read `git log` and the diffstat of the range the note names first — the stages before it are done, and their work is in those commits and in the note, never in your context.
