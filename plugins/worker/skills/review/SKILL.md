...
7. Record the summary, which the pull request stage reads from a fresh context that cannot see yours. First make sure the gate record still answers for HEAD (`gate.sh print`): the pull request brief reads that same record, so a commit made after the last gate run has to be gated before the hand-over. Then pipe only the `disputed:` lines that still stand — `disputed: none` when none do — into `"${CLAUDE_PLUGIN_ROOT}/scripts/panel.sh" record`, again with a quoted heredoc. It derives `review_rounds:`, the `panel:` line and the summed `fixed:` counts from the round records, closes them, and prints the summary it recorded. Report that summary to the user, not your memory of the rounds.
