...
2. The gate runs once per round, and only here: no reviewer runs it. The injected block above is this round's gate when it reads `pass` for this head. Otherwise run `"${CLAUDE_PLUGIN_ROOT}/scripts/gate.sh" run` after the commit of step 1, so the record names the commit the reviewers read, and take the block from `"${CLAUDE_PLUGIN_ROOT}/scripts/gate.sh" print`. Launch no reviewer until that block reads `pass` for this head: a failed gate is fixed and committed first, and a block that says `none for this head` means the gate has not run on what the reviewers would read.
