...
- Make the smallest complete change that closes the issue. Reproduce bugs end to end before fixing. Run the gate through the worker's `gate.sh run` after committing, never `make check` by hand: it records the result for that commit, and that record is what the reviewers are briefed with. Fix lint, test failures and flakiness you meet along the way.
