...
- A report that stops for a person opens with `blocked:` and what you need on that one line, with the detail after it; a finished one opens with `ready: <pr-url>`. The factory reads the outcome from that line, so a question without the word reads as a failed run.
