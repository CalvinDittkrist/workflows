...
5. Create it: `gh pr create --base <base> --title ... --body-file <tmp>` (use `npx -y gh-axi pr create` if gh-axi is available; same flags). Add `--draft` when `panel_verdict: draft`, whatever an override says, and name in the body which reviewers did not pass; the draft is how a panel that did not pass reaches a person, and only the maintainer lifts it, after reading the body.
