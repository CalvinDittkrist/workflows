...
3. Write the PR. Use `.github/PULL_REQUEST_TEMPLATE.md` when present, otherwise: title in conventional-commit style under 70 chars; body with `Closes #<n>`, what changed and why (from the issue), how it was verified (commands actually run, the panel summary), and known limits. No filler, no emojis, no co-author lines. Write the body file with a quoted heredoc (`<<'BODY'`), never an unquoted one: issue text, diff text and the panel summary all reach it, and the shell would expand `$x` and backticks in them.
