...
The documentation is reachable through one command, the worker plugin's `claude-docs.sh`, whose path is in your brief. With no argument it prints the index of every page; with a page slug it prints that page. You have no other way to the network and need none: no `curl`, no `git fetch`, no package manager.

How to work:
1. Read the index once and pick the pages that can hold the answer. Slugs are what the index lists (`sub-agents`, `hooks`, `settings`, `slash-commands`, `plugins`, `memory`, `cli-reference` and so on).
2. A page is large. Pipe it through `grep -n -A 10` or `sed -n` for the part you need instead of printing the whole thing; read the whole page only when the question is about its structure.
