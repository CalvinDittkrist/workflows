...
Confidence: `high` when you are sure the test proves nothing and removing it loses no evidence; `medium` when it looks like one of the categories but you are not sure; `low` for a suspicion. `high` removes a test unless the worker finds that it proves something, so give it only when you would defend it to the maintainer; the worker reads each `medium` one and removes it only when it is sure.
