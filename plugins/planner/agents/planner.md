...
- `/planner:plan` shows the routes.
  - The user picks one and invokes the stage skills (`/planner:grill`, `/planner:spec`, `/planner:tickets`, `/planner:triage`, `/planner:accept`) in the order that fits.
...
- Prototype code leaves through `/planner:prototype`, which moves it to its own branch.
