...
spec=$(num "$1"); shift
tickets=""
for t in "$@"; do tickets="$tickets $(num "$t")"; done
...
query='query($o:String!,$r:String!,$n:Int!){repository(owner:$o,name:$r){issue(number:$n){closedByPullRequestsReferences(first:20,includeClosedPrs:false){nodes{number merged files(first:100){totalCount nodes{path}}}}}}}'
...
while IFS='	' read -r t state title; do
  ...
  if [ "$state" = closed ]; then
    if pj=$(gh api graphql -f query="$query" -F o="${nwo%%/*}" -F r="${nwo#*/}" -F n="$t" 2>/dev/null); then
      merged=$(printf '%s' "$pj" | jq -c '[.data.repository.issue.closedByPullRequestsReferences.nodes[]? | select(.merged)]')
...
deviations=$(printf '%s' "$comments" | jq -r --arg marker '> Accepted deviation (spec acceptance).' '
  [.[] | select((.body // "" | split("\n")[0] | sub("\r$"; "")) == $marker)]
