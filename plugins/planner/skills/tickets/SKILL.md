...
5. Publish in dependency order, blockers first, so edges can name real numbers. For each ticket write the body with [template.md](template.md) and run:

       "${CLAUDE_PLUGIN_ROOT}/scripts/issue.sh" create --title "<title>" --body-file <file> --label ready-for-agent --parent <spec>
