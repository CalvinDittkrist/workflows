...
Labels. Category: `bug` or `enhancement`, exactly one. State: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human` or `wontfix`, exactly one. If an issue carries two states, say so and ask before doing anything else. Every comment you post starts with `> Written by an agent during triage.`
...
5. Apply the outcome with `"${CLAUDE_PLUGIN_ROOT}/scripts/issue.sh"`:
   - `ready-for-agent`: post the brief from [brief.md](brief.md) with `comment`, then `label <n> --add ready-for-agent --remove needs-triage`.
   - `ready-for-human`: the same brief plus one line on why it cannot be delegated.
...
6. When the user says "move #n to <state>", confirm the change in one line and do it. Skip the interview, but offer a brief when the target is `ready-for-agent`.
