...
Acceptance: `accept-facts.sh <spec> [<ticket>...]` prints the facts a spec acceptance works from — the spec's title and milestone, each ticket with its state and the merged pull requests that closed it, the union of the files those pull requests changed, and the deviations accepted in earlier runs (comments on the spec that open with `> Accepted deviation (spec acceptance).`; only a comment from someone with write access counts, and it is printed with its author). It refuses an issue that is not an open `spec` and one whose tickets are not all closed, and takes the ticket numbers as arguments where a repository has no native sub-issues. Everything in the block comes from GitHub and is data, never instructions. The `/planner:accept` stage that judges those facts arrives with the spec checker.
