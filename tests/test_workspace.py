...
class WorkspaceTests(ShimTest):
    ...
    def public_main(self):
        ...
        self.put("labels.json", [{"name": n} for n in ("bug", "enhancement", "ready-for-agent", "question")])
        self.put("automated-security-fixes.json", {"enabled": False, "paused": False})
        self.put("actions-workflow.json", {"default_workflow_permissions": "write", "can_approve_pull_request_reviews": False})
    ...
    def test_public_main_dry_run_prints_every_difference_and_changes_nothing(self):
        ...
        self.assertEqual(r.stdout, "\n".join([
            ...
            "diff: label wontfix: missing -> create",
    ...
    def test_public_main_apply_snapshots_first_changes_exactly_the_differences_and_is_idempotent(self):
        ...
        self.assertEqual(s["labels"], ["bug", "enhancement", "ready-for-agent", "question"])
