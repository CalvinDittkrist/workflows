...
class SessionStartHookTests(ShimTest):
    ...
    def test_non_issue_branch_and_subagent_are_silent(self):
        ...
        # A test hunt's branch names no issue: without a handoff note waiting, its session hears nothing.
        self.assertEqual(self.hook("hunt/tests-2026-09-24").stdout, "")
...
class HuntBranchBriefTests(ShimTest):
    """A test hunt's branch names no issue, so the scripts that print one say none, and the diff context the
    reviewers and the pull request author read carries the hunt record in its place (ADR 0045)."""
    ...
    def test_the_facts_and_the_diff_context_say_none_for_the_issue(self):
        facts = self.run_script(WORKER / "facts.sh", WF_BASE_BRANCH="main")
        self.assertEqual(facts.returncode, 0, facts.stderr)
        self.assertIn("\nissue: none", facts.stdout)
        ctx = self.run_script(WORKER / "diff-context.sh", WF_BASE_BRANCH="main")
        self.assertEqual(ctx.returncode, 0, ctx.stderr)
        self.assertIn("issue: none", ctx.stdout)
        self.assertNotIn("issue: #", ctx.stdout)
        self.assertIn("hunt_removed: 0", ctx.stdout)
...
class HandoffTests(ShimTest):
    ...
    def test_a_test_hunt_gets_its_note_although_its_branch_names_no_issue(self):
        self.git("checkout", "-qb", "hunt/tests-2026-09-24")
        self.assertEqual(self.handoff().returncode, 0)
        ctx = self.hook()
        self.assertIn("test hunt on hunt/tests-2026-09-24", ctx)
        self.assertIn("the note lives in the worktree's git directory", ctx)
        self.assertIn("**review** stage", ctx)
        self.assertFalse([c for c in self.calls() if c.startswith("gh issue")], "a hunt reads no issue")
        facts = self.facts()
        self.assertIn("issue: none", facts)
        self.assertIn("resume_stage: review", facts)
        self.assertEqual(self.hook(), "", "the note goes to one context, and the hunt has nothing else to say")
...
class HuntPathsTests(ShimTest):
    ...
    conventions, a large one split by file count."""
    ...
    def test_every_convention_is_found_and_other_files_are_not(self):
        ...
        self.assertIn(": ui/src, files: 2\n", out)

    def test_a_large_directory_is_split_into_bounded_shares(self):
        self.commit({f"tests/test_{i:02}.py": "" for i in range(23)})
        out = self.shares()
        self.assertIn("hunt_shares: 3,", out)
        self.assertIn("share 1: tests, part 1 of 3, files: 10\n", out)
        self.assertIn("share 3: tests, part 3 of 3, files: 3\n", out)
        self.assertEqual(out.count("  file: "), 23)
...
"""
    ...
    def test_a_triaged_reply_names_the_removals_keeps_the_medium_ones_and_refuses_what_does_not_fit(self):
        self.hunt("round")
        reply = "\n".join([
    ...
    def test_the_hunt_runs_three_rounds_at_most(self):
        for n in range(3):
            ...
            self.hunt("removed", stdin=f"remove: tests/test_x{n}.py | test_x | cannot-fail | no assertion\n"
