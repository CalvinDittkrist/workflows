...
| context value | The size of a worker session's context, written by the status line of its pane to `<worktree git dir>/worker/context` (`total_input_tokens`, `context_window_size`, the time of the reading) and read by the worker's `checkpoint.sh`; the only contract between the orchestrator and the worker plugin ([ADR 0020](adr/0020-the-pane-measures-the-context-and-the-worktree-carries-the-value.md)). |
| handoff | Continuing a worker's issue in a fresh context because its own has grown past `WF_HANDOFF_TOKENS` (default 120 000). `checkpoint.sh` answers `handoff: yes \| no \| unavailable`; it reports only, and no stage acts on the verdict yet. |
...
| factory | The service that works routed issues unattended on a host of its own, as a second driver over the worker pipeline ([ADR 0021](adr/0021-the-factory-is-a-second-driver-over-the-worker-pipeline.md)). A Go service in `factory/`, not a plugin. |
| factory host | The dedicated machine the factory runs on; it shares nothing with a developer's machine but GitHub, and it is the factory's isolation boundary ([ADR 0026](adr/0026-the-factorys-isolation-boundary-is-the-host.md)). |
...
| queue | The routed issues of all connected repositories in one line, derived from GitHub on every poll, never stored; work in progress comes before new issues ([ADR 0024](adr/0024-one-queue-one-worker-work-in-progress-first.md)). |
...
| remote claim | Creating the issue's branch through the GitHub API, which exactly one claimer wins ([ADR 0023](adr/0023-a-claim-is-the-creation-of-the-branch-through-the-api.md)). |
