...
| bot review signal | A review by a Bot account on a held issue's pull request, after the last run ended, that leaves an unresolved thread. It is to queue a follow-up run whose repair count carries over, which a later ticket implements ([ADR 0051](adr/0051-a-bots-review-queues-a-follow-up-run-within-the-repair-budget.md)). |
