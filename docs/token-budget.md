...
- `/repo-standards:standardize` is `disable-model-invocation: true` too. Plugin agents cannot be hidden, so the six auditor descriptions (one sentence each) are listed in every session that enables `repo-standards`: worker sessions keep it for `/repo-standards:adr` and carry those six lines, planner sessions start with it disabled.
