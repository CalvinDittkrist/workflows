...
## Projects
One GitHub project per product repository, copied from a template project, with status Triage, Ready, In progress, In review, Done and priority P0 to P3. When the repository has no project linked and `WF_PROJECT_TEMPLATE=<owner>/<number>` names the template, `workspace.sh --apply` copies it and links the copy. Settings the API cannot set are printed as `manual:` steps: the API cannot create or turn on project workflows, and a copy leaves out the template's auto-add workflow, so auto-add is turned on by hand.
...
Approval is per category, never per finding ([ADR 0016](adr/0016-approval-is-per-category-and-scripts-own-what-they-apply.md)). Within an approved category the run performs `delete` and `issue` findings one by one, exactly as the report lists them. `configure`, `create` and `replace` findings describe what a script works out for itself: approving a category scaffolds every missing baseline file of it, and approving `workspace` applies the whole difference between the GitHub workspace and the standard. The report keeps the two groups apart and says so per category.
