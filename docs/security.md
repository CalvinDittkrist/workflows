...
- That is surface reduction, not containment: measured in a live worker session on 2026-09-21, a subagent whose own file declares `WebFetch` gets it even though the worker's tool list has neither web tool, so a subagent's declared tools are granted rather than intersected with the parent's. The enforced network boundary is the permission layer and the sandbox `allowedDomains` above; the pinned script keeps the untrusted-text context away from the open web and gives the pipeline one auditable command instead of a free fetch tool. The lookup agent itself holds `Bash`, like every reviewer and auditor, so its read-only, one-origin behaviour rests on its prompt plus the permission layer, not on its tool list ([ADR 0029](adr/0029-agents-verify-claude-code-facts-against-the-live-documentation.md)).
