...
- A host that loads the worker with `--plugin-dir` in `worker_args` records no worker version and gets a warning per run.

## Consequences
- Every run names its worker plugin, Claude Code and factory, on the record and the dashboard.
- The `workflows` marketplace becomes a trust boundary of every host.
