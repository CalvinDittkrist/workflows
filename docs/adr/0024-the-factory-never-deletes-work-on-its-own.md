# 0024. The factory never deletes work on its own
...
Every branch the factory leaves behind blocks a later claim on that issue ([ADR 0022](0022-a-claim-is-the-creation-of-the-branch-through-the-api.md)), which is what makes the release signal and `--force` adoption the two ways back in.
