...
- Every new form of that line, such as a heading or a list item, was a parser case.
...
- A failed process, a missing result line or a result that does not fit the schema is a failed run.
- `blocked` is a blocked run with its summary as the reason.
- `complete` with a pull request of the run's repository is a ready run; the URL is rebuilt from repository and number.
- The factory checks the result against the schema itself.
- A session past its stage's `timeout` is ended with its process group.

## Consequences
- The report parser goes away, and the coupling moves to the documented result line.
- A new field changes the schema, the reader and the prompts at once.
- Rejected: hardening the report parser, which grows with every wording a model finds.
