...
## Consequences
The pull request body carries the panel result verbatim, including a panel that ended on FIX and every `disputed:` line, and a panel that did not pass opens the pull request as a draft that only the maintainer lifts. The pipeline driver reports the same recorded text instead of quoting its memory. In yolo mode that draft ends the run: `finish.sh` refuses it and says so, because the point of the draft is that a person reads the body, and an autonomous run is exactly where nobody would.
