...
The worker reports nothing to the factory. Everything the factory knows about a run it reads from the stream: the stages from the `Skill` calls, a subagent's events from the `parent_tool_use_id` they carry, the outcome from the final report, which is markdown written for a person, and cost, turns and token totals from the result line, which is the only line that carries them. A worker behaves the same whether a person or the factory started it: the one thing the pipeline owes the factory is that its final report names the outcome, `ready:` as the work skill already says and `blocked:`, which the ticket that starts real workers adds to it.
...
The parser is the coupling: the factory depends on the shape of Claude Code's stream and on a worker ending with a report that says `ready:` or `blocked:`, which is the one demand it makes of the pipeline. A change to either makes runs read as `failed` without the worker being wrong, which is why the stream shape is recorded with the version it was taken from and the scripted worker of fake mode replays exactly that shape.
