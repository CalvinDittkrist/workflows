...
16. Removing the routing label or closing the issue cancels a run. A run ending without a pull request, or let go, pushes its worktree, then removes it when let go and keeps it otherwise ([ADR 0026](adr/0026-the-factory-never-deletes-work-on-its-own.md)).
