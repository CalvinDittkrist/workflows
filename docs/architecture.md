...
16. Removing the routing label or closing the issue cancels a run. A run ending without a pull request pushes its worktree's HEAD to its branch. Letting go pushes the worktree before removing it ([ADR 0026](adr/0026-the-factory-never-deletes-work-on-its-own.md)).
