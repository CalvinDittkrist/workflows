...
| `worker_args` | `[]` | Arguments added to the worker's `claude` command line, such as `["--model", "opus"]`. `--settings`, `--agents`, `--agent`, `--permission-mode`, `--output-format`, `-p` and `--print` are refused: the factory sets them. The read-only sessions, the author of the pr stage and the reviewers of the review stage, take only the `--model` of these, and a reviewer whose own model is named takes none. |
