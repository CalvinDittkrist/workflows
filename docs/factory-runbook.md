...
## Identity
The host is the factory's isolation boundary ([ADR 0027](adr/0027-the-factorys-isolation-boundary-is-the-host.md)). Its workers run unattended in the auto permission mode, so anything on the host is theirs to read and run.

- Keep nothing on the host that is not already on GitHub.
- Give it no credential of yours beyond the Claude subscription the workers run on.
- No SSH key of yours, no Claude Code login of yours and no `gh` login of yours go on the host.

1. **A GitHub machine user.** Create a GitHub account for the host, such as `yourname-factory`.
   - Its claims, commits, pull requests and comments are its own, so you can review a pull request you did not write.
   - Invite it as a collaborator with the Write role to each repository the factory is to work, and to nothing else.
   - It creates branches, assigns issues to itself, pushes and opens pull requests.
2. **A token limited to the connected repositories.** Sign in as the machine user and create a personal access token.
   - For repositories an organisation owns, use a fine-grained token with that organisation as the resource owner and *Only select repositories*.
   - Give it Contents, Issues and Pull requests read and write, and Metadata read.
   - A fine-grained token cannot reach the repositories of a personal account the machine user is only a collaborator on.
   - There a classic token with the scopes `repo`, `read:org` and `workflow` works.
   - It reaches the repositories the machine user was invited to, which is why step 1 invites it to nothing else.
   - `workflow` lets a worker push a change to a repository's `.github/workflows`, which a change to CI needs.
...
1. **The user and its directories.**
   ...
   `gh` comes from GitHub's own apt repository ([install_linux.md](https://github.com/cli/cli/blob/trunk/docs/install_linux.md)), not from the distribution.

   - Debian trixie ships 2.46.0, which still asks for Projects (classic) in `gh pr edit`.
   - GitHub answers that query with the error `Projects (classic) is being deprecated in favor of the new Projects experience`.
   - So with that version the factory's `gh pr edit --add-reviewer` fails, and a run that ends `ready` asks nobody for a review.
   - The keyring is installed only when its checksum matches the one that page lists.
   - apt upgrades the package from that repository along with the rest of the system.
...
2. **The gate's tools.** The factory runs each connected repository's `make check`, unless its gate runs on CI (`gate.command`).
   ...
   - Each archive and the NodeSource key are installed only when `sha256sum --check` answers `OK`. So those lines are chained with `&&`, since a pasted block runs on past a failed line.
   - Node comes from NodeSource's apt repository for the major version CI pins, `node_24.x`, the way `gh` comes from GitHub's.
   - NodeSource publishes no checksum for its key. The one above is the sha256 of the key with the fingerprint `6F71 F525 2828 41EE DAF8 51B4 2F59 B5F9 9B1B E0B4`.
   - When NodeSource rotates the key, `gpg --show-keys nodesource-repo.gpg.key` shows the new fingerprint. Check it against theirs before the checksum is replaced.
   - apt upgrades Node within that major version; moving to another major version is a new source line.
   - `/usr/local/go/bin` is not on a login's `PATH`, so the service puts it there (see [Service](#service)).

   staticcheck is built with that Go as the user `factory`, into `~/go/bin`, where the `Makefile` looks for it:
   ...
   - Run it only where no parent directory is writable by anyone but root.
   - npx runs the `node_modules/playwright` of the nearest parent that has one.
   - The workers write to `/tmp` and to the clones under `/var/lib/factory`, so from there root would run what a worker left.
   - When CI moves a pin, move the host's in the same way before the next run.

   The gate's length tells you whether a host is fast enough.

   - The factory runs the gate once in the gate stage, and again on the final head when the review's fixes moved the branch.
   - It runs once more for every fix session a failing gate takes ([The gate stage](#the-gate-stage)).
   - Each run may take `gate.timeout` (default `45m`) before it is ended and counts as a failure.
   - On a Raspberry Pi 4 this repository's full gate takes an estimated 12 to 15 minutes.
   - A slow host costs time, and blocks a run only past `gate.timeout`.
   - To judge a host, time the gate as the user `factory` in a clone of each connected repository, with the service's `PATH`: `time env PATH="/usr/local/go/bin:$PATH" make check`.
3. **Claude Code**, as the user `factory` (`sudo -iu factory`), with the native installer. It needs no Node and puts `claude` in `~/.local/bin` ([setup](https://code.claude.com/docs/en/setup.md)):
...
4. **The headless login**, as the user `factory`: run `claude`, type `/login` and sign in with the subscription the workers run on.
   - Over SSH the browser cannot reach the host. Claude Code shows a URL to open on another machine and a code to paste back ([authentication](https://code.claude.com/docs/en/authentication.md)).
   - The credential lands in `~/.claude/.credentials.json`, mode 0600, where the worker sessions renew it.
   - The quota check renews it too when it finds it expired ([ADR 0037](adr/0037-the-quota-check-waits-below-12-percent-of-the-workers-scope.md)).
...
7. **quota-axi** in a pinned version. The factory reads the output of quota-axi 0.1.49 ([ADR 0037](adr/0037-the-quota-check-waits-below-12-percent-of-the-workers-scope.md)).
   ...
   - With 0.1.50 the check waits while the window is fresh and starts runs when it is nearly used up.
   ...
   - NodeSource's npm has the prefix `/usr` (`npm prefix -g`). Its global packages land in `/usr/lib/node_modules`, with their commands in `/usr/bin`.
   - So the path for `quota_axi` is `/usr/bin/quota-axi`.
   - A Node with the prefix `/usr/local`, such as one from the tarball on nodejs.org, puts it in `/usr/local/bin/quota-axi` instead.
...
| `quota_minimum` | `12` | The percentage of a scope below which no run starts; `0` never waits. See below the table. |
...
- `-config <file>`, by default `factory.json` in the working directory.
...
The dashboard is then at `https://<host>.<tailnet>.ts.net/` for the devices of your tailnet.
...
### Cancelling
- The worktree's commits are pushed before the worktree is removed.
- A push the remote refuses because the branch moved on counts as landed when the fetched branch contains the worktree's HEAD.
- That is the case after you merged the base in, or fixed a review on the pull request.
- Any other refused push keeps the worktree, the branch and the assignee until the commits are on the remote.
...
Its brief names the issue, the repository, the branch and the base it was cut from. The session:

- reads the issue with `gh`, its body cut at 6,000 characters and its last eight comments at 1,500 each
- reads the repository's `AGENTS.md` or `CLAUDE.md`
- makes the smallest complete change and updates the docs it makes stale
- checks it with the single tests of the files it touched
- commits in conventional commits without a co-author
- never pushes and runs no full gate, reviewer or pull request: those are the stages after it

It reports `complete` with its commits, and the run goes on to the gate stage. A session that reports `complete` on a branch with no commit beyond its base fails the run, since there is nothing to gate.

It reports `blocked` when the issue cannot be done without you. The run ends `blocked` with the session's summary, what it needs and why, as its reason and in the comment on the issue.

### The gate stage
Once the implement session has reported its commits, the factory runs the gate itself.

- When the base has commits the branch lacks, it first merges `origin/<base>` into the branch with a merge commit, never a rebase.
- A merge that conflicts is left in progress and goes to a fix session given the conflicted files. The session resolves them and commits the merge.
- A session that gives the merge up rather than committing it fails the run.

The factory then determines the change class ([Change classes](#change-classes)) and runs its gate, `make check` for the class `full`.

- The gate runs in the worktree, in a process group of its own, for at most `gate.timeout`.
- A gate past it is ended with its process group and counts as a failure.
- A failure goes to a fix session given the end of the gate's output. The gate runs again on the commit the session leaves.
- That repeats up to `gate.rounds` fix sessions. Past that the run is `blocked`, and its reason names the failure with the end of the output.
- A class without a gate runs none and says so.

Every run of the gate is recorded on the run under `gates`:

- its stage, commit, class, command and exit status
- whether it timed out, its duration in seconds and the end of its output
...
At the first gate of the run on CI:

- The factory pushes the branch and opens a draft pull request against the base, and records it on the run.
- The draft is titled after the issue, with the body `Closes #N` and nothing else.
- It then reads the checks of the pushed head every `poll`, as the ci stage does, and the run's stage reads `gate`.
- A check still running is waited for. The checks passing is the gate's pass.
- With every check read, the pass stands only on two readings a poll apart that show the same checks.
- GitHub registers the checks of a head one workflow at a time, and a quick one can pass before a slow one appears.
- The pass is recorded under `gates` with each check read, and handed to the reviewers as the gate result.

A check that fails is a failed gate:

- A fix session is given the failed checks with the end of their failed logs.
- The factory pushes its commit and reads the checks of the new head.
- The budget is that of a gate in the worktree: `gate.rounds` in the gate stage and `review.gate_rounds` on the final head.
- Past it the run is `blocked`, and its reason names the failing checks.
- A named check that has not appeared `ci.checks_grace` after the push ends the run `blocked`, naming what is missing. So does a head with no check.
- So a check whose name is misspelt never reads as a pass.
...
Before it opens one, the factory reads the branch's open pull requests:

- One it opened itself for this run is taken over.
- Any other ends the run `blocked`, with a comment on the issue naming it. That includes one of the machine user that the run did not record.
- Delete that pull request or the branch, and route the issue again.

Bot reviewers and the draft:

- Bot reviewers that review on their own, such as Codex, skip drafts. The draft gets its bot review once the pr stage marks it ready.
- The ci stage waits for that review then, as for any pull request.
- A workflow of the repository that runs on `ready_for_review` starts checks of its own once the draft is marked ready.
- In a repository whose workflows name that event, the ci stage waits for them.
- It reads the draft's checks as a pass only once a check has finished since, or `ci.checks_grace` has passed.
- A bot set to review drafts too reviews the gate's commits early. It costs repair rounds on findings the review stage fixes anyway, so set it to skip drafts.

Restarts and gestures:

- Marking the draft ready by hand changes nothing. The factory reads the stage from its record.
- So the run still goes through the review and the pr stage, and the rounds it spends are still the gate's.
- A factory restarted while it waits on the gate's checks resumes at the gate stage with the recorded draft.
- One restarted after the gate passed resumes at the review stage. Neither opens a second pull request.
- Taking the routing label off during a fix session of the gate ends that session with its process group. It gives the issue back, as in every stage.
...
- A finding has a severity: `S1` must fix, `S2` should fix, `S3` a nit.
- It has a path, a line, the claim, why it is wrong and the fix.
- The findings decide the verdict: `fix` when an `S1` or `S2` stands, `pass` otherwise.
- A reviewer that reports the other verdict is read by its findings. The run carries a warning naming the reviewer and the round.
- A result that does not fit its schema, such as a finding without a claim, fails the run and names the reviewer.

When any reviewer says `fix`, one fix session in the worktree is given every finding of the round, numbered `F1`, `F2` and so on.

- It fixes or disputes, with a reason, every `S1` and `S2`, and may skip an `S3` with a reason.
- It commits and reports what it fixed, disputed and skipped.
- A report that leaves an `S1` or `S2` out fails the run, and one that says `blocked` blocks it.
- The next round runs only the reviewers whose last verdict was `fix`. A round in which all of them pass ends the review.
- After `rounds` rounds a `fix` that still stands ends the review all the same. The panel did not pass, and the pull request says so.
...
- A failure goes to a fix session with the end of the gate's output, and the gate runs again, up to `gate_rounds` fix sessions.
- Past that the run is `blocked` with the output in its reason.
...
- a `name`
- `paths`: patterns relative to the repository's root, whose parts between slashes are `path.Match` patterns or `**` for any number of directories
...
- The class is determined when the review starts, and the reviewers of the first round are its reviewers, kept for a resumed review.
- It is determined again on the final head before every gate there, since a fix can change a file the first class does not cover.
- That gate runs the class's command. A class without a gate runs none, and the gate result says `gate_result: none` and `gate_command: none` and names the class.
- The gate stage determines it the same way before each of its gates.

Every determination is logged with what decided it: the class that covers every file, or the first file outside every class. It is recorded on the run's panel, and listed on the `change_class:` line of the panel summary in the pull request. The dashboard shows it beside the review stage.

CI still runs the whole gate on every pull request, so what a class leaves out fails there and costs a repair round.

A class is refused on start, with the fix, when:

- it has no name or no paths, or a pattern that is no pattern
- its gate is left out or is none of the four forms, or a reviewer is not one of the five
- it is named `full`, or its name is used twice

### The pr stage
Once the review stage has ended, the factory opens the pull request itself. It pushes the branch, then starts a read-only author session in the run's worktree:
...
- the gate result
- the panel summary it derived from the recorded rounds: every reviewer's verdicts over the rounds and the fixes by severity
- every dispute word for word, with the finding it disputes
- When the panel did not pass, the section says so and names the reviewers that did not pass.

The pull request goes against the base the branch was cut from. It is never left a draft, since no bot reviews one: you decide on it.
...
### The ci stage
Once the pr stage has opened the pull request, the factory waits on it, reading it every `poll`. It reads mergeability first, then the checks, then the review of a configured bot. Then it reads the reviews and unresolved threads of the pull request, and acts on the first row that decides:

| The pull request | The factory |
| --- | --- |
| conflicts with the base | merges the base into the branch in the run's worktree (a merge commit, never a rebase). A clean merge is pushed as it is. A conflicting one is left in progress, and a fix session in the worktree given the conflicted files resolves, commits and pushes. |
...
- A reply to a thread the brief did not list is posted nowhere and warned about.
- A thread whose reply went through but whose resolution failed is resolved on the next reading, without another session. So it gets its reply once.
- Nobody dismisses a review. A review stands on GitHub until its author approves.
- Once answered, the run does not read it as asking again. Nor does a later run on the same pull request, whichever spelling of the repository its record keeps.

Rounds:

- Every merge, fix session and address-reviews session is one repair round, counted against `repair_rounds`.
- A red round of a gate on CI before the pull request is ready spends the gate's budget instead.
...
- The factory judges the pull request again once its head has moved off the commit the round was spent on.
- A fix session that reports `blocked` blocks the run on its words.
- The ci stage runs inside the run's `deadline`, and a cancel ends a fix or address-reviews session like any session.
...
- `WF_CI_REPAIR_ROUNDS`, `WF_PR_BOT_REVIEWERS` and `WF_PR_REVIEW_WAIT` are `ci.repair_rounds`, `ci.bot_reviewers` and `ci.review_wait`.
- `WF_REVIEWERS` and `WF_REVIEW_ROUNDS` are `review.reviewers` and `review.rounds`.
- A variable with no knob of its own set something no session of the factory reads any more.

Pause the factory, write those knobs and remove `worker_env` from `/etc/factory/factory.json`. Install the new binary as in [Updating](#updating) and unpause it.
...
### Pausing
Set `"paused": true` in the configuration and save it; the factory reads it at its next poll, with no restart.

- A run that is going finishes with its own outcome, notified like any ending.
- After it the paused factory shows the line, polls GitHub and writes nothing there.
...
`-paused` is the brake no configuration undoes. Add it to `ExecStart` with `systemctl edit factory` and restart. The factory stays paused whatever the file says, until you take it out and restart again.
...
- Most of what a host writes comes from the gate, not the factory. An event log is a few KB per run.
- The gate's caches live in the home of the user `factory` and in the worktrees.
- They write about 1 GB on the first run and 100 to 150 MB per run afterwards.
...
- Release one by taking the assignee off. The factory takes the branch up again by itself when three things hold.
- The branch carries commits beyond the base, the machine user pushed it last, and no pull request of it is open.
- It assigns itself, makes the worktree from the branch and continues on those commits ([ADR 0024](adr/0024-a-claim-is-the-creation-of-the-branch-through-the-api.md)).
- A branch somebody else pushed last is a foreign claim. So is one with an open pull request, or with nothing beyond the base.
- Then the run ends `lost` and touches nothing.
- Delete such a branch, or finish the issue by hand, and set the routing label again.
- A lost run stands in the way of no routing newer than it.
