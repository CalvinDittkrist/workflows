...
func TestAReviewThatAsksForChangesRunsTheWorkerOnItInTheSameWorktree(t *testing.T) {
	...
	// where a first run stands at implement and goes through the review and pr stages into the ci stage
	// the factory runs.
	if !equal(first.Stages, []string{"implement", "review", "pr", "ci"}) || !equal(second.Stages, []string{"address-reviews", "ci"}) {
		t.Errorf("the two runs went through the stages %v and %v, want [implement review pr ci] and [address-reviews ci]", first.Stages, second.Stages)
	}
