...
// stateChangesRequested is what GitHub calls a review that asks for changes.
const stateChangesRequested = "CHANGES_REQUESTED"
...
// newestRequest reads the one pull request. Every failure ends the reading, because a review list
// that was only half read is not one the newest review can be taken from, and a run is queued on
// what GitHub said rather than on a gap in it.
func (g *gitHub) newestRequest(ctx context.Context, repository string, pull int) (time.Time, error) {
	...
	if err != nil {
		return none, fmt.Errorf("the pull request %s could not be read: %w", pullKey(repository, pull), err)
	}
	if strings.TrimSpace(string(state)) != "open" {
		// Merged or closed: what a review asked of it is not this factory's to answer, and it is not
		// asked about again. That bound is what keeps the watch to the pull requests that are open:
		// the factory holds every issue it has ever claimed until cleanup lets one go ([ADR 0026]),
		// and a call per poll for each of them would spend a rate limit on work that is over. It is
		// memory and no file, like every reading of GitHub, so a pull request somebody reopens is
		// watched again from the next start of the factory.
		g.done(pullKey(repository, pull))
		return none, nil
	}
	raw, err := gh(ctx, "api", "--paginate", reviewsRequest(repository, pull))
	if err != nil {
		return none, fmt.Errorf("the reviews of %s could not be read: %w", pullKey(repository, pull), err)
	}
	newest := none
	...
	for {
		...
		if err := decoder.Decode(&page); err != nil {
			...
			return none, fmt.Errorf("the reviews of %s are no review list: %w", pullKey(repository, pull), err)
		}
		for _, review := range page {
			// The newest of them is the answer, whatever order GitHub sends them in, and the write
			// access of its author is asked about only for a review that would queue a run.
