...
// built-in class full applies, the repository's single gate and the review's panel. It determines it
...
// fullGate is the gate of the class full: the repository's single gate ([ADR 0008]).
//
// [ADR 0008]: ../docs/adr/0008-make-check-is-the-single-gate.md
var fullGate = []string{"make", "check"}
...
// out and one that is not a list are told apart from an empty list, which is a class without a gate.
...
// pattern, a gate that is a list of arguments, and reviewers the review has.
...
// readGate is a class's gate command: a list of arguments, the program first, and no gate at all when
// the list is empty. It is run as it stands, without a shell.
func readGate(name string, raw json.RawMessage) ([]string, error) {
	fix := `write it as a list of arguments, such as ["make", "docs"], or [] for no gate`
	if len(bytes.TrimSpace(raw)) == 0 {
		return nil, fmt.Errorf("the class %q has no gate; %s", name, fix)
	}
	var gate []string
	if err := json.Unmarshal(raw, &gate); err != nil || gate == nil {
		return nil, fmt.Errorf("the gate of the class %q is %s, which is not a list of arguments; %s", name, bytes.TrimSpace(raw), fix)
	}
	if len(gate) > 0 && strings.TrimSpace(gate[0]) == "" {
		return nil, fmt.Errorf("the gate of the class %q names no program; %s", name, fix)
	}
	return gate, nil
}
...
// class decided — the gate command, empty for none, and the reviewers.
type Classed struct {
...
func (f *Factory) determine(ctx context.Context, r *Run, entry Entry, claim claimed, panel *Panel, knobs reviewSettings, what string) (Classed, error) {
	...
	class, why := classify(knobs.Classes, files, knobs.Reviewers)
	...
	gate := "no gate"
	if len(class.Gate) > 0 {
		gate = "the gate " + commandLine(class.Gate)
	}
