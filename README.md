...
| `WF_REVIEW_MANDATE` | set by the factory's follow-up run | says the session was started to answer a review somebody asked for, which is what `repair.sh reset` starts the pull request's repair count again on; unset on every other session |
